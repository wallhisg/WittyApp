    ...
    env_(env)
{
  ...
  ServerWidget *chatWidget =
      new ServerWidget(server_, root());
  ...
  b->clicked().connect(this, &ServerAppication::addChatWidget);
}

void ServerAppication::javaScriptTest()
{
  if(!env_.javaScript()){
    ...
    timer_->timeout().connect(this, &ServerAppication::emptyFunc);
    timer_->start();
  }
}

void ServerAppication::emptyFunc()
{}

void ServerAppication::addChatWidget()
{
  ServerWidget *chatWidget2 = new ServerWidget(server_, root());
...
{
public:
  ChatWidget(const WEnvironment& env, Server& server);

private:
  JSignal<WString> login_;
};

ChatWidget::ChatWidget(const WEnvironment& env, Server& server)
  : WApplication(env),
    login_(this, "login")
{
  setCssTheme("");
...
WApplication *createWidget(const WEnvironment& env, Server& server)
{
